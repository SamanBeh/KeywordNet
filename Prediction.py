...
def dataframe_to_folds(dataframe, folds_count):
    fold_size = int(len(dataframe) / folds_count)
    start_point = 0
    folded_list = []
    for i in range(folds_count):
        if i == folds_count-1:
            folded_list.append(dataframe.iloc[start_point:])
        else:
            folded_list.append(dataframe.iloc[start_point: start_point + fold_size])
        start_point += fold_size
    return folded_list

def get_train_test_folds(folded_list, test_fold_index):
    train = pd.DataFrame(columns=list(folded_list[0]))
    test = pd.DataFrame(columns=list(folded_list[0]))
    for i in range(len(folded_list)):
        if i == test_fold_index:
            test = test.append(folded_list[i])
        else:
            train = train.append(folded_list[i])
    return train, test
...
positive_data = pd.read_csv("/home/saman/KW/nap/jour/{}s/diff-{}-{}.txt".format(step, first_file, second_file))
negative_data = pd.read_csv("/home/saman/KW/nap/jour/{}s/miss-{}-{}.txt".format(step, first_file, second_file))
...
output_df_path = "jour/{}s/preds/pred-it{}-{}-{}.csv".format(step, iteration, first_file, second_file)
log_file_name = "jour/{}s/preds/log-it{}-{}-{}.csv".format(step, iteration, first_file, second_file)
...
selected_cols = ["Source","Target",\
"S-degree","T-degree",\
"S-ClustCoef","T-ClustCoef",\
"S-DegCent", "T-DegCent",\
"S-EigenCent","T-EigenCent",\
"S-ClosenessCent","T-ClosenessCent",\
"S-BetweenCent","T-BetweenCent",\
"S-PageRank","T-PageRank",\
"JaccCoeff","RACoeff",\
"KNLP_DegCent","KNLP_EigenCent","KNLP_ClosenessCent","KNLP_BetweenCent","KNLP_PageRank",\
"KNLPC_DegCent","KNLPC_EigenCent","KNLPC_ClosenessCent","KNLPC_BetweenCent","KNLPC_PageRank",\
"community_score"]
...
positive_data_main = dataframe_to_folds(positive_data_main, K)
negative_data_main = dataframe_to_folds(negative_data_main, K)
...
for i in range(K):
    ...
    positive_data, pred_pos = get_train_test_folds(positive_data_main, i)
    ...
    negative_data, pred_neg = get_train_test_folds(positive_data_main, i)
        ...
        if algorithm.lower() == "knn":
            for j in range(2,7):
                ...
                if j == 2:
                    ...
                    if save_models.lower() in ["1", "yes", "y", "yeah", "whatever"]:
                        save_model_pickle(knn, root_dir, file_name="knn-{}.pickle".format(i))
                    elapsed_time = datetime.now() - start_time
                    print("\n  * Elapsed time: {}".format(elapsed_time))
                    print("-------------------------")
                    with open(log_file_name, "a") as file:
...
output_df_path = output_df_path[["tp","tn","fp","fn","sensitivity","specificity","precision","npv","fnr","fpr","fdr","forate","acc","f1score","mcc", "auc"]]
